...
async def process_text(credentials: HTTPAuthorizationCredentials=Depends(HTTPBearer())):
    ...
    # key_to_check = ["user_id","username","email","role"]
    # value_to_check = [420,'jaydeep.dixit','jaydeep@flyfare.in','admin']
    # count =0 
    # for i in range(len(key_to_check)-1):

    #     if key_to_check[i] in user and user[key_to_check[i]] == value_to_check[i]:
    #         count+=1
    # if count ==3:
    if user:
        response_front_end=[{
